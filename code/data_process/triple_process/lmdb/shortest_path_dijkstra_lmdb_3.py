...
if __name__ == '__main__':
    file_path = '/data/LLMKG/CTKG/data/lmdb/train2id.txt'
    # file_path = '/data/LLMKG/CTKG/code/data_process/triple_process/test.txt'
    table_path = "/data/LLMKG/CTKG/data/lmdb/shortestpath_table/"
    # shutil.rmtree(table_path)
    # os.mkdir(table_path)
    graph = read_graph_from_file(file_path)

    for start_node in range(123000,134999):
        distances, predecessors = dijkstra(graph.copy(), start_node)
        print(distances)
        save_array_to_txt(distances,"/data/LLMKG/CTKG/data/lmdb/shortestdistance_table/"+str(start_node//100*100)+"-"+str((start_node//100+1)*100-1)+".txt")
        save_array_to_txt(predecessors,"/data/LLMKG/CTKG/data/lmdb/shortestpath_predecessor/"+str(start_node//100*100)+"-"+str((start_node//100+1)*100-1)+".txt")
        if start_node%10 == 0:
